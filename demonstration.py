...
def load_demonstrations ( path ):
    ...
    # 데이터 폴더 내의 모든 파일을 순회
    frame_files = sorted([f for f in os.listdir(path) if f.startswith('state') or f.startswith('action')])
    
    for file_name in frame_files:
        file_path = os.path.join(path, file_name)
        if file_name.startswith('state'):
            observations.append(np.load(file_path, allow_pickle=True))
        elif file_name.startswith('action'):
            actions.append(np.load(file_path, allow_pickle=True))
...
def record_demonstration ( path = "scenarios" ):
